...
void drawTitleScreen()
{
  ...
  for (uint8_t i = 0; i < 4; i++) sprites.drawSelfMasked(32 * i, 0, titleScreen, i);
  sprites.drawSelfMasked(85, 45, badgeMysticBalloon, 0);
  sprites.drawSelfMasked(79, 43, stars, sparkleFrames);
  sprites.drawSelfMasked(9, 9, leftGuyLeftEye, pgm_read_dword(&blinkingEyesLeftGuy[blinkingFrames])&255);
  sprites.drawSelfMasked(15, 13, leftGuyRightEye, pgm_read_dword(&blinkingEyesLeftGuy[blinkingFrames])&255);
  sprites.drawSelfMasked(109, 34, rightGuyEyes, pgm_read_dword(&blinkingEyesRightGuy[blinkingFrames])&255);
}
...
void stateMenuHelp()
{
  for (uint8_t i = 0; i < 2; i++) sprites.drawSelfMasked(32, 32 * i, qrcode, i);
  if (arduboy.justPressed(A_BUTTON | B_BUTTON))
  {
    gameState = STATE_MENU_MAIN;
    sound.tone(425, 20);
