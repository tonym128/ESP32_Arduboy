...
void stateGameNextLevel()
{
  ...
  //{
    if (arduboy.everyXFrames(20))
    {
      ...
      if (coinsCollected > 0)
      {
        coinsCollected--;
        scorePlayer += 20;
        sound.tone(pgm_read_dword(tones + toneindex++)&255, 150);
      }
      ...
      else
      {
        ...
        {
          sound.tone(pgm_read_dword(tones + toneindex++)&255, 200);
  ...
  {
    EEPROM.put(OFFSET_LEVEL, (uint8_t)LEVEL_TO_START_WITH - 1);
