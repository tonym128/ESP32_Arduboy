...
void checkKid()
{
  ...
  // Update position---
  // -Solid checking
  int16_t tx = (kid.pos.x + 6) >> 4;
  int16_t ty = (kid.pos.y + 8) >> 4;
  ...
  int16_t tx2 = (((kid.actualpos.x + kid.speed.x) >> FIXED_POINT) - 1 + (kid.speed.x > 0) * 14) >> 4;
  boolean solidH = gridGetSolid(tx2, (kid.pos.y + 2) >> 4)
                   || gridGetSolid(tx2, (kid.pos.y + 13) >> 4);
  int16_t ty2 = (((kid.actualpos.y - kid.speed.y) >> FIXED_POINT) + (kid.speed.y < 0) * 17) >> 4;
  ...
  if (kid.balloons == 0 || kid.speed.y > 0 || !solidbelow)
  {
    ...
    if (kid.isBalloon)
    {
      ...
      {
        //kid.speed.y = maxVal(-((8 / kid.balloons) >> 1), kid.speed.y);
        kid.speed.y = maxVal(kid.balloons - 5, kid.speed.y);
      }
    }
  }
...
void updateCamera()
{
  ...
  vec2 V;
  //vec2 V = (kid.pos - cam.pos + cam.offset) >> 3; // more uint8_ts
  V.x = kid.pos.x - cp.x - 58;
  V.y = kid.pos.y - cp.y - 24;
  V = V >> 2;

  cam.pos += V;
  cam.pos.y = minVal(320, cam.pos.y);
}
...
void drawKid()
{
  if (kid.isActive)
  {
    ...
    if (kid.isBalloon)
    {
      int16_t commonx = kidcam.x - (6 * kid.direction);
      int16_t commony = kidcam.y + kid.balloonOffset;
      if (kid.balloons > 1)
      {
