...
void enemiesUpdate()
{
  ...
  if (key.active)
  {
    int16_t commonx = key.pos.x - cam.pos.x;
    int16_t commony = key.pos.y - cam.pos.y;
  ...
  for (uint8_t i = 0; i < MAX_PER_TYPE; ++i)
  {
    ...
    if (fans[i].active)
    {
      ...
      // Draw fan
      int16_t _x = fans[i].pos.x - cam.pos.x;
      int16_t _y = fans[i].pos.y - cam.pos.y;
