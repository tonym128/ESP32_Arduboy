...
void drawTextBox(byte x, byte y, boolean color)
{
  ...
  for (byte i = 1; i < ((rollText) ? (textRollAmount + 1) : (textBox[0] + 1)); i++)
  {
    if (textBox[i] == NEWLINE)
    {
      ...
      if (color) sprites.drawSelfMasked(x + xOffset, y + yOffset, font_, textBox[i]);
      else sprites.drawErase(x + xOffset, y + yOffset, font_, textBox[i]);
      xOffset += 6;
    }
  }
}
...
void drawYesNo()
{
  ...
  sprites.drawSelfMasked( 98, 50 + (6 * !cursorYesNoY), font_, 44);
}
