...
static const Rect screenRect = {0, 0, WIDTH, HEIGHT};
...
static bool updateEnemies(void)
{
    ...
    for (int i = 0; i < ENEMY_MAX; i++, pE++) {
        ...
        for (SHOT_T *pS = shots; pE->life > 0 && pS < &shots[SHOT_MAX]; pS++) {
            ...
            tmpPoint.x = pS->x;
            tmpPoint.y = pS->y;
            if (pS->x < WIDTH && arduboy.collide(tmpPoint, enemyRect)) {
                pS->x = WIDTH;
                pE->life--;
                isDamaged = true;
            }
...
static Point getEnemyCoords(GROUP_T *pG, ENEMY_T *pE, uint16_t frames)
{
    ...
    Point tmpPoint;
    tmpPoint.x = pE->bx + WIDTH / 2 + myCos(xDeg, pG->xRad);
    tmpPoint.y = pE->by + mySin(yDeg, pG->yRad);
    return tmpPoint;
}
...
static bool updateBullets(void)
{
    bool ret = false;
    Rect playerRect;
    playerRect.x = playerX / PLAYER_SCALE - 1;
...
static Point getBulletCoords(BULLET_T *pB)
{
    ...
    Point tmpPoint;
    tmpPoint.x = pB->bx + myCos(pB->deg, rad);
    tmpPoint.y = pB->by + mySin(pB->deg, rad);
    return tmpPoint;
}
