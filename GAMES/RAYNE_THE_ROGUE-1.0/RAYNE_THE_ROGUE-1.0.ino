...
void sfs::GetFileAddress() {
 /* for (uint16_t i = 16; i < 1018; i++) {
    ...
    if (EEPROM.read(i) == fileName[0] &&
      ...
      EEPROM.read(i+5) == fileName[5])z*/
      startAddress = 20;
      fileSize = EEPROM.read(20+6);
...
//initialize
const char collect_treasures[]  = "COLLECT TREASURES";
const char move_to_exit[]  = "MOVE TO DOOR TO EXIT";
const char dodge_dragon_attacks[]  = "DODGE DRAGON ATTACKS";
const char collect_key[]  = "COLLECT KEY TO UNLOCK DOOR";
const char select_a_save[]  = "SELECT A SAVE SLOT";
const char select_string[]  = "SELECT";
...
//player
 const unsigned char rayne_l1[] = { 0x00,0xC0,0x60,0x3C,0x3F,0xEB,0x88,0x00 }; 
 const unsigned char rayne_l2[] = { 0x00,0x00,0x60,0xFC,0xBF,0x0B,0x08,0x00 }; 
 const unsigned char rayne_l3[] = { 0x00,0x00,0xC0,0xBC,0x7F,0x6B,0x08,0x00 }; 
 const unsigned char rayne_r1[] = { 0x00,0x88,0xEB,0x3F,0x3C,0x60,0xC0,0x00 }; 
 const unsigned char rayne_r2[] = { 0x00,0x08,0x0B,0xBF,0xFC,0x60,0x00,0x00 }; 
 const unsigned char rayne_r3[] = { 0x00,0x08,0x6B,0x7F,0xBC,0xC0,0x00,0x00 }; 
...
const unsigned char *rayne_left_ani[] = {rayne_r1,rayne_r2,rayne_r3};
 ...
 const unsigned char rayne_death_1[] = { 0x00,0x88,0xEB,0x3F,0x3C,0x60,0xC0,0x00 }; 
 const unsigned char rayne_death_2[] = { 0x00,0x44,0x68,0x3B,0xFF,0x00,0x00,0x00 }; 
 const unsigned char rayne_death_3[] = { 0x10,0x52,0x74,0x3C,0x0F,0x03,0x00,0x00 }; 
 const unsigned char rayne_death_4[] = { 0x08,0x28,0x3A,0x1C,0x18,0x0E,0x06,0x00 }; 
 const unsigned char rayne_death_5[] = { 0x08,0x28,0x38,0x18,0x1E,0x18,0x0C,0x0C }; 
 const unsigned char rayne_death_6[] = { 0x10,0x50,0x70,0x30,0x3C,0x30,0x18,0x18 }; 
 const unsigned char rayne_death_7[] = { 0x20,0xA0,0xE0,0x60,0x70,0x60,0x30,0x30 }; 
 const unsigned char rayne_death_8[] = { 0xC0,0xC0,0xC0,0xC0,0xE0,0xC0,0x60,0x60 };
 const unsigned char rayne_death_9[] = { 0xC0,0xC0,0xC0,0xC0,0xC0,0x80,0xC0,0xC0 }; 
 const unsigned char rayne_death_10[] = { 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 }; 
 const unsigned char invisibility_1[] = {0x7E, 0x81};
...
//dragons
 ...
 const unsigned char acid_dragon_1[] = { 0xF0,0xF4,0xF4,0xF9,0xFA,0xFC,0xFD,0xF6,0x74,0x7E,0x3C,0x1C,0x1C,0x00,0x00,0x00 }; 
 const unsigned char acid_dragon_2[] = { 0xF0,0xF4,0xF4,0xF9,0xFA,0xFC,0xFD,0xF6,0x74,0x7E,0x6C,0x4C,0x4C,0x00,0x00,0x00 }; 

 const unsigned char fire_dragon_1[] = { 0xF8,0xF8,0x7D,0x3D,0x3F,0x7F,0x7E,0x7E,0xD6,0x9C,0x8C,0x1C,0x08,0x00,0x00,0x00 }; 
 const unsigned char fire_dragon_2[] = { 0xF8,0xF8,0x7D,0x3D,0x3F,0x7F,0x7E,0x7E,0x76,0x3C,0x3C,0x1C,0x08,0x00,0x00,0x00 };

 const unsigned char frost_1[] = { 0xF8,0xF8,0x7D,0x3D,0x3F,0x7F,0x7E,0x7E,0xD6,0x9C,0x8C,0x1C,0x08,0x00,0x00,0x00 }; 
 const unsigned char frost_2[] = { 0xF8,0xF8,0x7D,0x3D,0x3F,0x7F,0x7E,0x7E,0x76,0x3C,0x3C,0x1C,0x08,0x00,0x00,0x00 };

 const unsigned char snow_flake_1 [] = {
...
const unsigned char *cold_ani[] = {snow_flake_1,snow_flake_1};
 ...
 const unsigned char fire_1[] = { 0x48,0x12,0x48,0x9D,0x7F,0xFE,0x7D,0x2C };
 const unsigned char fire_2[] = { 0x48,0x12,0x48,0x9D,0x7F,0xFE,0x7D,0x2C }; 
const unsigned char *fire_ani[] = {fire_1,fire_2};

 const unsigned char acid_spit_1[] = { 0x02,0x03,0x03,0x01,0x03,0x02,0x03,0x03,0x03,0x03,0x03,0x02,0x03,0x03,0x01 }; 
const unsigned char *acid_ani[] = {acid_spit_1,acid_spit_1};


//treasure
 const unsigned char chest4[] = { 0x7E,0x4B,0x55,0x4B,0x4F,0x7F,0x49,0x3E }; 
 const unsigned char chest3[] = { 0x7E,0x4B,0x57,0x4B,0x4F,0x7F,0x49,0x3E }; 
 const unsigned char chest2[] = { 0x7E,0x4F,0x55,0x4F,0x4F,0x7F,0x49,0x3E }; 
 const unsigned char chest1[] = { 0x7E,0x4F,0x5D,0x4F,0x4F,0x7F,0x49,0x3E }; 
 const unsigned char pouch1[] = { 0x20,0x76,0x7D,0x75,0x7D,0x7D,0x76,0x20 }; 
 const unsigned char pouch2[] = { 0x20,0x76,0x7D,0x55,0x7D,0x7D,0x76,0x20 }; 
 const unsigned char pouch3[] = { 0x20,0x76,0x6D,0x5D,0x6D,0x7D,0x76,0x20 }; 
 const unsigned char pouch4[] = { 0x20,0x76,0x6D,0x55,0x6D,0x7D,0x76,0x20 }; 
 const unsigned char bag4[] = { 0x72,0xDB,0xAF,0xDE,0xFA,0x70,0x00,0x00 }; 
 const unsigned char bag3[] = { 0x72,0xDB,0xBF,0xDE,0xFA,0x70,0x00,0x00 }; 
 const unsigned char bag2[] = { 0x72,0xFB,0xAF,0xFE,0xFA,0x70,0x00,0x00 }; 
 const unsigned char bag1[] = { 0x72,0xFB,0xEF,0xFE,0xFA,0x70,0x00,0x00 }; 
 const unsigned char coin4[] = { 0x1C,0x36,0x6B,0x77,0x7F,0x3E,0x1C,0x00 }; 
 const unsigned char coin3[] = { 0x1C,0x36,0x6F,0x77,0x7F,0x3E,0x1C,0x00 }; 
 const unsigned char coin2[] = { 0x1C,0x3E,0x6B,0x7F,0x7F,0x3E,0x1C,0x00 }; 
 const unsigned char coin1[] = { 0x1C,0x3E,0x7B,0x7F,0x7F,0x3E,0x1C,0x00 }; 
 const unsigned char diamond[] = { 0x18,0x3C,0x7E,0xFF,0x7E,0x3C,0x18,0x00 }; 
 const unsigned char lamp[] = { 0x01,0x07,0x2C,0x3E,0x3E,0x2F,0x09,0x0F }; 
 const unsigned char harp[] = { 0x3F,0x44,0x7E,0x44,0x7E,0x44,0x3F,0x00 }; 
 const unsigned char candleabra[] = { 0x07,0x44,0x66,0x7C,0x66,0x44,0x07,0x00 }; 




//objects
 const unsigned char key[] = { 0x00,0x00,0xA6,0xA9,0xF9,0x06,0x00,0x00 }; 
 const unsigned char closeddoor[] = { 0xFE,0xEF,0xFF,0xFF,0xFF,0xFF,0xFE,0xB4 }; 
 const unsigned char opendoor[] = { 0xFE,0x03,0x01,0x01,0xFD,0xEF,0xFE,0xB4 }; 

//rings
 const unsigned char aring[] = { 0x00,0x2E,0x53,0x59,0x53,0x2E,0x00,0x00 }; 
 const unsigned char cring[] = { 0x00,0x2E,0x51,0x55,0x55,0x2E,0x00,0x00 }; 
 const unsigned char iring[] = { 0x00,0x2E,0x55,0x51,0x55,0x2E,0x00,0x00 }; 
 const unsigned char lring[] = { 0x00,0x2E,0x51,0x57,0x57,0x2E,0x00,0x00 }; 
 const unsigned char pring[] = { 0x00,0x2E,0x51,0x59,0x59,0x2E,0x00,0x00 }; 

//potions
 const unsigned char small_potion_a[] = { 0x00,0x30,0x7A,0x4E,0x66,0x4A,0x30,0x00 }; 
 const unsigned char small_potion_c[] = { 0x00,0x30,0x7A,0x46,0x56,0x7A,0x30,0x00 }; 
 const unsigned char small_potion_i[] = { 0x00,0x30,0x7A,0x7E,0x46,0x7A,0x30,0x00 }; 
 const unsigned char small_potion_l[] = { 0x00,0x30,0x7A,0x46,0x5E,0x5A,0x30,0x00 }; 
 const unsigned char small_potion_p[] = { 0x00,0x30,0x7A,0x46,0x66,0x7A,0x30,0x00 }; 


 const unsigned char lpotion[] = { 0x10,0x38,0x45,0x5F,0x5D,0x38,0x10,0x00 }; 
 const unsigned char ipotion[] = { 0x10,0x38,0x55,0x47,0x55,0x38,0x10,0x00 }; 
 const unsigned char ppotion[] = { 0x10,0x38,0x45,0x67,0x65,0x38,0x10,0x00 }; 
 const unsigned char apotion[] = { 0x10,0x38,0x4D,0x67,0x4D,0x38,0x10,0x00 }; 
 const unsigned char cpotion[] = { 0x10,0x38,0x45,0x57,0x55,0x38,0x10,0x00 }; 
...
const unsigned char *img_objects[] = {
 ...
 const unsigned char rayne_portrait [] = {
