...
//  guarantee of support in the future. The code is also free of signed charge in order to play. This
...
const unsigned char title_char_head[]  = {
...
const unsigned char title_char_body[]  = {
...
const unsigned char title_start[]  = {
...
const unsigned char castinglabel[]  = {
...
const unsigned char swaplabel[]  = {
	0x18, 0x1a, 0x02, 0x08, 0x0b, 0x03,
};
const unsigned char vol[2][8]  = {
	{
...
const unsigned char mesa[2][128]  = {
	{
...
const unsigned char mesa_back[2][27]  = {
	{
...
const unsigned char cowboy[2][2][2][32]  = {
	{
		//BACK
		{
			//STANDING
...
const unsigned char whip[2][4][2][21]  = {
	{
		//BACK
		{
			//1
...
const unsigned char border[4][32]  = {
	{
...
const unsigned char highscorelabel[]  = {
...
const unsigned char newhighscorelabel[]  = {
...
const unsigned char levellabel[]  = {
...
const unsigned char gameoverlabel[]  = {
...
const unsigned char heart[2][6][10]  = {
	{
		//BACK
...
const unsigned char snake[2][2][32]  = {
	{
		//BACK
		{
...
const unsigned char coyote[2][2][2][32]  = {
	{
		//BACK
		{
			//STANDING
...
const unsigned char barrel[2][2][32]  = {
	{
		//BACK
		{
...
const unsigned char tumbleweed[2][4][32]  = {
	{
		//BACK
		{
...
const unsigned char boss_bull_legs[2][32]  = {
	{
		//Stand
...
const unsigned char boss_bull_hit[]  = {
...
const unsigned char boss_bull_stand[]  = {
...
const unsigned char boss_bull_legs_back[2][34]  = {
	{
		//Stand
...
const unsigned char boss_bull_hit_back[]  = {
...
const unsigned char boss_bull_stand_back[]  = {
...
const unsigned char boss_tornado[4][32]  = {
	{
		//Bottom 1 (BL)
...
const unsigned char boss_tornado_back[4][36]  = {
	{
		//Bottom 1 (BL)
...
const unsigned char cactus[2][32]  = {
	{
		//BACK
...
const unsigned char tiles[2][56][8]  = {
	{
		//BACK
		//SOLID
...
const unsigned char levelparts[4][PARTS][8][PART_WIDTH]  = {
	{
		//LOW1
		{
...
signed char menustate = STATE_PROGRAMMING;
signed char level = 1;
signed char levellength = 1;
signed char sectionheight = LOWEST;
int score = 0;
int highscore = 0;
...
signed char playerstate = PLAYER_FALLING;
signed char playerdirection = RIGHT;
byte lives = STARTLIVES + 1;
signed char health = MAXHEALTH;
unsigned char jumptimer = JUMPMAX;
unsigned char falltimer = 0;
unsigned char runtimer = 0;
const signed char jumpcurve[JUMPCURVEMAX + 1] = { 3, 3, 3, 2, 2, 2, 2, 1, 1 };
const signed char fallcurve[FALLCURVEMAX + 1] = { 3, 2, 2, 2, 1, 1, 1, 1, 1 };
const signed char runcurve[RUNCURVEMAX + 1] = { 1, 1, 1, 1, 1, 2, 2, 2, 2 };
unsigned char hurttimer = 0;
unsigned char whiptimer = 0;
unsigned char deathcounter = 0;
signed char playerx = 0;
signed char playery = 0;
signed char whipx = 0;
signed char whipy = 0;
...
signed char bossx = 0;
signed char bossy = 0;
signed char bosshurt = 0;
signed char bosshealth = -1;
bool bosstype = true;
...
bool mesatype = 0;
signed char mesadistance = 119;
signed char moondistance = 90;
signed char screenshake = 0;

//Sound Functions
const byte musicfight[]  = {
...
const byte *currentsong = musicbackground;
...
void drawlevel() {
	for(signed char y = 0; y < 8; y++) {
		for(signed char x = 0; x < PART_WIDTH * 3; x++) {
...
void drawui() {
	...
	//Draw boss health
	if(bosshealth > 0) {
		for(signed char i = 0; i < bosshealth; i++) {
...
//Object Handling
signed char spawn(signed char object, unsigned char x, unsigned char y) {
	signed char i = 0;
...
void hotswapparts() {
	for(signed char y = 0; y < 8; y++) {
		for(signed char x = PART_WIDTH; x < PART_WIDTH * 3; x++) {
...
void generatepart(signed char section) {
	bool ending = false;
	signed char part = random(0, PARTS);
	...
	for(signed char y = 0; y < 8; y++) {
		for(signed char x = 0; x < PART_WIDTH; x++) {
			signed char tile = pgm_read_byte(&levelparts[sectionheight][part][y][x]);
			if(tile > BOUND_OTHER) {
				if(tile == HEART) {
					if(levellength % 7 == 2) {
...
void endgame() {
	if(score > highscore) {
		...
		highscore = score;
    EEPROM.put(SAVELOCATION+4, highscore);
    EEPROM.commit();
	} else {
...
//Movement Functions
signed char gettile(signed char x, signed char y) {
	if(y < 0) {
		return BLANK;
	}
	if(y >= 64) {
...
bool checkcollision(signed char x, signed char y, signed char boundry) {
...
void burst(signed char i) {
	enemies[i][OBJECT_TYPE] = OBJECT_POP;
	enemies[i][OBJECT_STATE] = 0;
}
void coyotelogic(signed char i) {
...
void bulllogic() {
	...
	if(bosstimer == 0) {
		...
		signed char safepath = (random(0, (BUFFER_BOSS / 16) - 1) + BUFFER_MED / 16) * 16;
		signed char i = 0;
		while(i < OBJECT_COUNT) {
...
void handleobjects() {
	...
	while(i < OBJECT_COUNT) {
		...
		signed char objecty = enemies[i][OBJECT_Y] - 6;
...
void inogamesetup() {
	...
	#endif
  uint32_t gamesignature;
	EEPROM.get(SAVELOCATION, gamesignature);
  EEPROM.get(SAVELOCATION+4, highscore);
	if(gamesignature != 64000) {
		highscore = 0;
    EEPROM.put(SAVELOCATION, (uint32_t)64000);
		EEPROM.put(SAVELOCATION+4, 0);
    EEPROM.commit();
	}
