...
void stateGameNew()
{
  ...
  for (byte i = 0; i < 36; i++)
  {
    sprites.drawErase( alphabetX, alphabetY, font_, i + 1);
    alphabetX += 12;
    if (alphabetX > 119)
    {
      alphabetY += 10;
  ...
  sprites.drawErase(6 + (cursorX * 12), 17 + (cursorY * 10), font_, 44);
};
...
void stateGameInventory()
{
  ...
  sprites.drawSelfMasked(86, 5 + (cursorY * 12), font_, 44);
};
...
void showSubMenuStuff()
{
  ...
  if (player.hasStuff[(2 * (gameState - STATE_GAME_ITEMS))])
    sprites.drawErase(5, 9 + (6 * cursorY), font_, 44);
