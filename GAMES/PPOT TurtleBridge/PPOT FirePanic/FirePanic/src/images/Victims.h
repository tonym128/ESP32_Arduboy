#pragma once

namespace Images {

  const uint8_t  Victim_OnEdge_01[] = {
  16, 11,
  0xc0, 0x20, 0x20, 0x10, 0xd0, 0xd0, 0x50, 0x18, 0x04, 0x02, 0x81, 0x61, 0x41, 0x82, 0x9c, 0x60, 
  0x00, 0x01, 0x01, 0x07, 0x01, 0x00, 0x00, 0x00, 0x04, 0x00, 0x03, 0x04, 0x00, 0x00, 0x00, 0x00, 
  };

  const uint8_t  Victim_OnEdge_01_Mask[] = {
  0xc0, 0xe0, 0xe0, 0xf0, 0xf0, 0xf0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff, 0x7f, 0x7f, 0xfe, 0xfc, 0x60, 
  0x00, 0x01, 0x01, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x04, 0x00, 0x00, 0x00, 0x00, 
  };

  const uint8_t  Victim_OnEdge_01_RH[] = {
  16, 11,
  0x60, 0x9c, 0x82, 0x41, 0x61, 0x81, 0x02, 0x04, 0x18, 0x50, 0xd0, 0xd0, 0x10, 0x20, 0x20, 0xc0, 
  0x00, 0x00, 0x00, 0x00, 0x04, 0x03, 0x00, 0x04, 0x00, 0x00, 0x00, 0x01, 0x07, 0x01, 0x01, 0x00, 
  };

  const uint8_t  Victim_OnEdge_01_RH_Mask[] = {
  0x60, 0xfc, 0xfe, 0x7f, 0x7f, 0xff, 0xfe, 0xfc, 0xf8, 0xf0, 0xf0, 0xf0, 0xf0, 0xe0, 0xe0, 0xc0, 
  0x00, 0x00, 0x00, 0x00, 0x04, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x07, 0x01, 0x01, 0x00, 
  };

  const uint8_t  Victim_OnEdge_02[] = {
  17, 7,
  0x3e, 0x41, 0x36, 0x41, 0x3e, 0x41, 0x55, 0x3e, 0x41, 0x5e, 0x3e, 0x41, 0x35, 0x0a, 0x2e, 0x51, 0x2e, 
  };

  const uint8_t  Victim_OnEdge_02_Mask[] = {
  0x3e, 0x7f, 0x3e, 0x7f, 0x3e, 0x7f, 0x7f, 0x3e, 0x7f, 0x7e, 0x3e, 0x7f, 0x3f, 0x0e, 0x2e, 0x7f, 0x2e, 
  };

  const uint8_t  Victims[] = {
  16, 15,
  , 
  };


  const uint8_t  Victims_Mask[] = {
  , 
  };

  const uint8_t  Victim_Halos[] = {
  12, 5,
  0x0e, 0x0a, 0x0e, 0x1f, 0x15, 0x1f, 0x1f, 0x15, 0x1f, 0x0e, 0x0a, 0x0e,  // Day
  0x00, 0x04, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x04, 0x00,  // Night
  
  0x00, 0x1c, 0x17, 0x1d, 0x1f, 0x17, 0x1d, 0x1f, 0x17, 0x1d, 0x07, 0x00,  
  0x00, 0x00, 0x08, 0x02, 0x00, 0x08, 0x02, 0x00, 0x08, 0x02, 0x00, 0x00, 

  0x00, 0x07, 0x1d, 0x17, 0x1f, 0x1d, 0x17, 0x1f, 0x1d, 0x17, 0x1c, 0x00, 
  0x00, 0x00, 0x02, 0x08, 0x00, 0x02, 0x08, 0x00, 0x02, 0x08, 0x00, 0x00, 
  };

  const uint8_t  Victim_Halos_Mask[] = {
  0x0e, 0x0e, 0x0e, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x0e, 0x0e, 0x0e, 
  0x00, 0x1c, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x07, 0x00, 
  0x00, 0x07, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1f, 0x1c, 0x00, 
  };

}