...
public class MainWindow extends JFrame
{
	...
	private void updateTextContents()
	{
		try {
			...
			w.println("const static  uint16_t LevelMap[] = {");
			boolean first = true;
			for (Level l: levels) {
				if (first) {
					first = false;
			...
			w.println(" */");
			w.println();
			w.println("const static  uint8_t BoxMap[] = {");
			for (Level l: levels) {
				w.println("    // " + l.levelName);
			...
			w.println(" */");
			w.println();
			w.println("const static  uint8_t EndMap[] = {");
			for (Level l: levels) {
			...
			w.println(" */");
			w.println();
			w.println("const static  uint8_t StartPos[] = {");
			for (Level l: levels) {
				if (l.person != null) {
