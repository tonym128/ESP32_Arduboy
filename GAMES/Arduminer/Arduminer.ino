...
long recipe_evaluation;
 ...
 const unsigned char state_idle = 1;
 const unsigned char state_walk = 2;
 const unsigned char state_action = 4;
 const unsigned char state_slash = 8;
#define state_crafting 16

 const unsigned char action_mine = 0;
 const unsigned char action_eat = 1;
 const unsigned char action_craft = 2;
 const unsigned char action_place = 3;
 const unsigned char action_sentinel = 4;
...
unsigned char save_game() {
  EEPROM.write(EEPROM_START+0, 0xBE);  
  EEPROM.write(EEPROM_START+1, 0x79);  
  EEPROM.write(EEPROM_START+2, 0x01);
  EEPROM.write(EEPROM_START+3, player.depth);
  EEPROM.write(EEPROM_START+4, player.health);
  EEPROM.write(EEPROM_START+5, (player.armour_weapon >> 8)&0xFF);
  EEPROM.write(EEPROM_START+6, player.armour_weapon&0xFF);
  EEPROM.write(EEPROM_START+7, player.stars_kills);

  for(unsigned char iter = 0; iter<16; iter++) {
    EEPROM.write(EEPROM_START+8+iter, inventory[iter]);  
  }
...
void draw_menu() {
  next_seed =random(65536);//+= blocks[next_seed % 20000];
  ...
  if(arduboy.pressed(DOWN_BUTTON) && j==4){
    ...
    main_menu_state &= 7;
    main_menu_state <<= 1;
    if (main_menu_state == 8) main_menu_state = MM_START;
    main_menu_state |= (l & MM_SOUND_BIT);
  }
  ...
  if(arduboy.pressed(UP_BUTTON) && j==4){
    ...
    main_menu_state &= 7;
    main_menu_state >>= 1;
    if (main_menu_state == 0) main_menu_state = MM_SOUND;
    main_menu_state |= (l & MM_SOUND_BIT);
  }

   arduboy.drawFastHLine(1,5,4,main_menu_state & MM_START ? WHITE : BLACK); 
   arduboy.drawFastHLine(1,13,4,main_menu_state & MM_LOAD  ? WHITE : BLACK); 
   arduboy.drawFastHLine(1,21,4,main_menu_state & MM_SOUND ? WHITE : BLACK); 
...
void set_attack() {
  ...
  for(unsigned char iter=0; iter<used_enemies; iter++) {
    ...
    signed char kk = a->p.ypos - player.p.ypos;
...
void handleEnemiesForCurrentState() {
  ...
  for(unsigned char iter=0; iter<used_enemies; iter++) {
    ...
    //update ypos info
    Enemy *a = &(enemy_buf[iter]);
    signed char kk = a->p.ypos - player.p.ypos + logical_next;
    if(kk > 64) { continue; }
    ...
    if(arduboy.everyXFrames(2)) {
      signed char vdist = a->p.xpos - player.p.xpos;
      if(vdist > 0) {
    ...
    if(arduboy.everyXFrames(player.depth < 10 ? 4 : player.depth < 20 ? 3 : player.depth < 40 ? 2 : 1)){
      ...
      signed char dmg = (j+1) * (player.depth < 10 ? 4 : player.depth < 20 ? 8 : player.depth < 40 ? 12 : 16);
...
void evaluate_recipes() {
  ...
  if( recipe_evaluation == 0xFFFFL ) {
    ...
    recipe_skip: 
    val = pgm_read_byte(recipes+i);
    while(val != 0xFF) {

      
      for(unsigned char iter = 1; iter <= (val & 0x7); iter++) {
        //recipes[i + iter] is the next element
        if(inventory_satisfies_element(pgm_read_byte(recipes + (i + iter))) == 0) {
...
void drawRecipeAtSlot(unsigned char offset, unsigned char index, unsigned char row_pos,unsigned char col_pos) {
    ...
    Recipe_Registry r = (Recipe_Registry)((pgm_read_byte(recipes+offset) >> 3) & 0x1F);
    
    //show required in top bar
    unsigned char val = 0;
    val = pgm_read_byte(recipes+offset);
    for(unsigned char elements=0; elements < (val & 0x7); elements++) {
        unsigned char element = pgm_read_byte(recipes + offset+1 + elements);
        //print icon.
