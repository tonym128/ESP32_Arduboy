...
void addBlockToInventory(unsigned char blockID) {
  ...
  switch(blockID){
    case block_stone:
      if(playSound()){ sound.tone(256,40); }
      blocksInInventory++;  
    ...
    case block_spawn: {
      ...
      if(playSound()){ sound.tone(10000,400); }
    }
    break;
    case block_spawn_no_loot: { 
      if(playSound()){ sound.tone(12000,400); }
      addItemToInventory(I_STEEL,1); 
    } break;
    case block_star: {
      add_star();  
    }
    default: if(playSound()){ sound.tone(256,40); } break;
  }  
}
...
void mine_state() {
  if(common_action(10)) {
    ...
    if(player.action_perf) {
      if(playSound()){ sound.tone(28000,120); } 
    }  
  }
}
  ...
  for(unsigned char iter=0; iter<used_enemies; iter++) {
    ...
    if(arduboy.everyXFrames(player.depth < 10 ? 4 : player.depth < 20 ? 3 : player.depth < 40 ? 2 : 1)){
      ...
      if(abs(kk) < 3 && dmg > 0) {
        ...
        if(playSound()){ sound.tone(750,20); }
        arduboy.setRGBled(255,0,0);
      }
    }
...
void evaluate_recipes() {
  ...
  if( recipe_evaluation == 0xFFFFL ) {
    ...
    recipe_skip: 
    val = pgm_read_byte_near(recipes+i);
    while(val != 0xFF) {

      
      for(unsigned char iter = 1; iter <= (val & 0x7); iter++) {
        //recipes[i + iter] is the next element
        if(inventory_satisfies_element(pgm_read_byte_near(recipes + (i + iter))) == 0) {
...
void drawRecipeAtSlot(unsigned char offset, unsigned char index, unsigned char row_pos,unsigned char col_pos) {
    ...
    Recipe_Registry r = (Recipe_Registry)((pgm_read_byte_near(recipes+offset) >> 3) & 0x1F);
    
    //show required in top bar
    unsigned char val = 0;
    val = pgm_read_byte_near(recipes+offset);
    for(unsigned char elements=0; elements < (val & 0x7); elements++) {
        unsigned char element = pgm_read_byte_near(recipes + offset+1 + elements);
        //print icon.
