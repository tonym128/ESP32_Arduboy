...
// サウンド
static const uint8_t soundOctaves[]  = {
  0x00, // O0
  0x0c, // O1
  0x18, // O2
  0x24, // O3
...
static const uint8_t soundLengths[]  = {
  0x01, // L0: 32分音符
  0x02, // L1: 16分音符
  0x03, // L2: 付点16分音符
  0x04, // L3: 8分音符
...
static const uint8_t soundNotes[]  = {
  0x09, // 'A'
  0x0b, // 'B'
  0x00, // 'C'
  0x02, // 'D'
...
static const uint16_t soundFrequencys[]  = {
...
// 画面キャプチャ
#if SYSTEM_DEBUG
static const char captureString[]  = "capture";
static const char captureHeader[]  = 
  "begin 644 capture.bmp\n"
  "M0DT^!````````#X````H````@````$`````!``$`````````````````````\n"
  "M`````````````````/___P";
static const char captureFooter[]  = 
  "\n`\nend\n";
#endif
